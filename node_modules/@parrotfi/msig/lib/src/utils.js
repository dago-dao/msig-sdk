...
async function fetchProposalsChainStates(multisigProg, proposals) {
    ...
    const chainTransactions = (await multisigProg.provider.connection.getMultipleAccountsInfo(txPubkeys)).map((acc, idx) => {
        return !acc
            ? null
            : {
                ...acc,
...
exports.sleep = sleep;
function buildMultisigProgram(rpc, multisigProgramId, wallet, opts = anchor_1.Provider.defaultOptions()) {
    const connection = new web3_js_1.Connection(rpc, opts.preflightCommitment);
    const provider = new anchor_1.Provider(connection, new NodeWallet(wallet), opts);
    return new anchor_1.Program(serum_multisig_idl_json_1.default, multisigProgramId, provider);
}
...
function setupJSONPrint() {
    web3_js_1.PublicKey.prototype["toJSON"] = function () {
        return this.toBase58();
    };
    web3_js_1.PublicKey.prototype[util_1.default.inspect.custom] = function () {
        return this.toBase58();
    };
}
